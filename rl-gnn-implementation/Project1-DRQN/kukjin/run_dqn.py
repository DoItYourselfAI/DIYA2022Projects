...
from model.DQN import ClassicCNN, train_dqn
...
def main():
    ...
    print(device)
    behavior_policy = ClassicCNN(4, 84, 84, 3, 2, 4).to(device)     # C, H, W, K, S, num_actions
    target_policy = ClassicCNN(4, 84, 84, 3, 2, 4).to(device) 
    ...
    for episode in range(total_episodes):
        ...
        while not done:
            env.render()
            action = behavior_policy.sample_action(torch.tensor(state).to(device), epsilon)
            ...
            if memory.size() > 2000:
                train_dqn(behavior_policy, target_policy, memory, optimizer,GAMMA, BATCH_SIZE)
